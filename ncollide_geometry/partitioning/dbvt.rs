...
struct DBVTInternal<P, B, BV> {
    ...
    /// This node parent.
    parent:          Arc<DBVTInternal<P, B, BV>>,
...
enum DBVTLeafState<P, B, BV> {
    ...
    RightChildOf(Arc<DBVTInternal<P, B, BV>>),
    /// This leaf is the left child of another node.
    LeftChildOf(Arc<DBVTInternal<P, B, BV>>),
...
impl<P, B, BV> DBVTLeafState<P, B, BV> {
    ...
    #[inline]
    fn unwrap(self) -> (bool, Arc<DBVTInternal<P, B, BV>>) {
        match self {
...
impl<P, BV, B> DBVTNode<P, B, BV>
    ...
    fn insert(self,
              ...
              -> Box<DBVTInternal<P, B, BV>> {
                  unimplemented!();
        // assert!(to_insert.read().unwrap().is_detached(), "Cannot insert the same node twice.");
