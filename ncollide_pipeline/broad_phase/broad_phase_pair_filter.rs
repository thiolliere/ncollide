...
pub trait BroadPhasePairFilter<P: Point, M, T, D>: Any + Send + Sync {
    /// Activate an action for when two objects start or stop to be close to each other.
    fn is_pair_valid(&self, b1: &CollisionObject<P, M, T>, b2: &CollisionObject<P, M, T>, filter_data: &D) -> bool;
}
...
pub struct BroadPhasePairFilters<P: Point, M, T, D> {
    filters: Vec<(String, Box<BroadPhasePairFilter<P, M, T, D>>)>,
}

impl<P: Point, M, T, D> BroadPhasePairFilters<P, M, T, D> {
    /// Creates a new set of collision filters.
    pub fn new() -> BroadPhasePairFilters<P, M, T, D> {
    ...
    pub fn register_collision_filter(&mut self, name: &str, callback: Box<BroadPhasePairFilter<P, M, T, D>>) {
    ...
    pub fn is_pair_valid(&self, b1: &CollisionObject<P, M, T>, b2: &CollisionObject<P, M, T>, filter_data: &D) -> bool {
        self.filters.iter().all(|&(_, ref f)| f.is_pair_valid(b1, b2, filter_data))
    }
}
